# Data Analysis for BMI, Hypertension, and GOSD

# Load GOSD data
gosd <- read.delim("C:/Users/BEHINLAPTOP/Desktop/gosd.txt")
gosd$nbr <- gosd$nbr + 3
print("Updated GOSD Data:")
print(gosd)

# Load BMI data
D <- file.choose()  # Prompt user to select BMI data file
dataBMI <- read.table(D, header = TRUE)
head(dataBMI)

# Attach dataset for direct column access
attach(dataBMI)
...
# Logistic regression with re-leveled SEX variable
...
# Logistic regression with additional AGE variable
